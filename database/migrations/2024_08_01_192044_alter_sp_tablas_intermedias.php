...
BEGIN
    ...
    insert into mml_cierre_ejercicio(clv_upp,estatus,statusm,ejercicio,capturista,created_at,updated_at,deleted_at,created_user,updated_user,deleted_user)
    select
        clave clv_upp,'Abierto' estatus,0 statusm,ejercicio,null capturista,
        now() created_at,now() updated_at,null deleted_at,
        usuario created_user,null updated_user,null deleted_user
    from catalogo
    where ejercicio = @anio_new and deleted_at is null and grupo_id = 6;
...
BEGIN
    ...
    set @tabla_exists := (select count(*) from comp_catalogo);
    ...
    set @query3 := concat(\"set @upp_old := (select distinct clv_upp from v_epp where ejercicio = \",@anio_old,\" and clv_upp = '\",upp_v,\"');\");
    ...
    if(@upp_old is null) then select 1 codigo; leave revision_upp; end if;
    if(@upp_new is not null) then select 2 codigo; leave revision_upp; end if;
    if(@tabla_exists is null) then select 3 codigo; leave revision_upp; end if;
    end if;
    ...
    select ne.id_old,ee.id id_new,ee.upp_id
    from new_entidad ne
    ...
    with new_conac as (
        ...
        select 
            ...
            c1.id_new padre_id,
            case
                when c2.id_new is null then 0
                else c2.id_new
            end tipologia_conac_id
        from conac c
        left join aux c1 on c.padre_id = c1.id_old
        left join aux c2 on c.tipologia_conac_id = c2.id_old
    ...
    left join (
        select id,padre_id,
            case 
                when tipologia_conac_id is null then 0
                else tipologia_conac_id
            end tipologia_conac_id
        from conac
        where ejercicio = @anio_new
    ) c on 
    c.padre_id = nc.padre_id and 
    c.tipologia_conac_id = nc.tipologia_conac_id;
    ...
    if(upp_v is null) then
        ...
        select * from epp
        where ejercicio = @anio_old and deleted_at is null;
    else 
        create temporary table epp_temporal
        select * from epp
    ...
    select 
        (@id_e:=@id_e+1) id,@anio_new ejercicio,1 mes_i,12 mes_f,et.upp_id,
        ad.id_new at_id,
        et.id_new et_id,
        ft.id_new ft_id,
        pt.id_new p_id,
        ...
        c2.id_new c2_id,
        c3.id_new c3_id,
        0 estatus,e.presupuestable,e.con_mir,0 confirmado,e.tipo_presupuesto,
        @created created_at,@created updated_at,null deleted_at,
        usuario created_user,null updated_user,null deleted_user
    from epp_temporal e
    ...
    if(upp_v is null) then
        create temporary table epp_temporal
        select * from epp where ejercicio = @anio_new;
    else 
        create temporary table epp_temporal
		select * from epp
		where ejercicio = @anio_new and upp_id = @id_upp_new;
    end if;
	...
	join (
        select 
			id,
			case 
				when tipologia_conac_id is null then padre_id
				else tipologia_conac_id
			end tipologia_conac_id
		from conac
    ) c on e.conac_id = c.id
    ...
    update upp_extras ue
    join (
        select distinct
            upp_id,clasificacion_administrativa_id 
        from epp where ejercicio = @anio_new
    ) ee on ue.upp_id = ee.upp_id and 
    ue.clasificacion_administrativa_id = ee.clasificacion_administrativa_id
    set ue.estatus_epp = 0;

    set @upp_mm := \"\";
    if(upp_v is not null) then set @upp:= concat(\" and clv_upp = '\",upp_v,\"'\"); end if;
   
    set @query := concat(\"
    insert into mml_avance_etapas_pp(clv_upp,clv_pp,etapa_0,etapa_1,etapa_2,etapa_3,etapa_4,etapa_5,estatus,ejercicio,nombre_minuta,ruta,extension,ramo33,created_at,updated_at,deleted_at,created_user,updated_user,deleted_user)
    select distinct
        clv_upp,clv_programa clv_pp,
        0 etapa_0,0 etapa_1,0 etapa_2,0 etapa_3,0 etapa_4,0 etapa_5,
        0 estatus,ejercicio,null nombre_minuta,null ruta,null extension,0 ramo33,
        now() created_at,now() updated_at,null deleted_at,
        \",usuario,\" created_user,null updated_user,null deleted_user
    from v_epp
    where ejercicio = \",@anio_new,\" and deleted_at is null\",@upp_m,\";
	 \");
