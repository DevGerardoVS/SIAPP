DELIMITER //
CREATE TRIGGER update_vepp_catalogo_on_sector_publico
AFTER UPDATE ON catalogo
FOR EACH ROW
begin
DECLARE sector_publico INT;
DECLARE epp_id INT;
declare done int DEFAULT 0;
-- Declarar el cursor
DECLARE epp_cursor CURSOR FOR 
SELECT id FROM epp WHERE clasificacion_administrativa_id  = sector_publico;
...
SELECT id INTO sector_publico 
FROM clasificacion_administrativa 
WHERE sector_publico_id  = OLD.id
...
v_epp
set
sector_publico = NEW.descripcion
where
