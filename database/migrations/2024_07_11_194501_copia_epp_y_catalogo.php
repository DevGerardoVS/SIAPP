...
BEGIN
    ...
    set @query4 := concat(\"set @upp_new := (select distinct clv_upp from v_epp where ejercicio = \",@anio_new,\" and clv_upp = '\",upp_v,\"');\");
    ...
    if(@upp_old is null) then select 1 codigo; end if;
    if(@upp_new is not null) then select 2 codigo; end if;
    if(@tabla_exists is null) then select 3 codigo; end if;

    if(@upp_old is not null and @upp_new is null and @tabla_exists is not null) then
