...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared("CREATE PROCEDURE mml_alineacion(in anio int,in trimestre_n int,in semaforo int,in corte varchar(13),in upp_v int)
BEGIN
	...
	select
		ose.clv_cpladem_linea_accion clv_cpladem,
		concat(c.clave,' ',c.descripcion) eje,
	...
	join (
		...
		where ejercicio = anio and deleted_at is null and grupo_id = 12
	) c on substr(ose.clv_objetivo_sectorial,1,1) = c.clave
	join (
		select
			case 
			...
			end clv_cpladem,descripcion linea_accion
		from catalogo
		where ejercicio = anio and deleted_at is null and grupo_id = 13
	) c2 on ose.clv_cpladem_linea_accion = c2.clv_cpladem;
	...
	from (
		...
		from metas
		where  ejercicio = \",anio,\" and deleted_at is null and mir_id is not null
	) m
	left join (
	...
	with aux as (
		select 
			t.clv_upp,ve.upp,t.clv_pp,ve.programa,t.clv_ur,ve.ur,clv_cpladem,componente_padre,nivel,tipo_indicador,objetivo,indicador,definicion_indicador,metodo_calculo,frecuencia_medicion,dimension,descripcion_metodo,medios_verificacion,total,avance
		from (
			select 
				mm.clv_upp,
				mm.clv_pp,
	...
	select 
		t.clv_upp,ve.upp,t.clv_pp,ve.programa,'' clv_ur,'' ur,clv_cpladem,componente_padre,nivel,tipo_indicador,objetivo,indicador,definicion_indicador,metodo_calculo,frecuencia_medicion,dimension,descripcion_metodo,medios_verificacion,total,avance
	from (
		select 
			clv_upp,
			clv_pp,
	...
	select 
		...
		descripcion_metodo,medios_verificacion,total,avance,ca.clv_cpladem
	from aux a 
	left join (
		select 
			clave,
	...
	select 
		...
		'' unidad_medida,
		dimension,
		medios_verificacion,total programado,avance,
		case 
