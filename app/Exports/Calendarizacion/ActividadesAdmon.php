...
class ActividadesAdmon implements FromCollection, ShouldAutoSize, WithHeadings, WithColumnWidths,WithTitle,WithStyles,WithEvents
{
    protected $filas;

    public function collection(){

        $data = MmlMirCatalogo::select('id AS clave','valor AS actividad')->where('deleted_at',null)->where('grupo','ActividadesGlobales')->get();
        $newData = [];
        foreach ($data as $key) {
             $a = array(
                 $key->clave,
