...
class ActividadesPpDelegacion implements FromCollection, ShouldAutoSize, WithHeadings, WithColumnWidths,WithTitle,WithStyles,WithEvents
{
    protected $filas;
    public function collection(){
        ...
        ->select(
            ...
            DB::raw('IF(enero>=1,2,"0") AS enero'),
            DB::raw('IF(febrero>=1,2,"0") AS febrero'),
            DB::raw('IF(marzo>=1,2,"0") AS marzo'),
            DB::raw('IF(abril>=1,2,"0") AS abril'),
            DB::raw('IF(mayo>=1,2,"0") AS mayo '),
            DB::raw('IF(junio>=1,2,"0") AS junio'),
            DB::raw('IF(julio>=1,2,"0") AS julio'),
            DB::raw('IF(agosto>=1,2,"0") AS agosto'),
            DB::raw('IF(septiembre>=1,2,"0") AS septiembre '),
            DB::raw('IF(octubre>=1,2,"0") AS octubre'),
