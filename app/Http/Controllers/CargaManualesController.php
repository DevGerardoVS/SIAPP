...
class CargaManualesController extends Controller
{
    ...
    public function getManuales(){
        try{
            ...
            foreach ($data as $d) {
                ...
                $button = '<a class="btn btn-sm btn_file_confirm" href="'.route('download_manual',['action'=>'download_file_confirm','id'=>$d->id]).'" title="'.__('messages.descarga_archivo_confirm').'"><i class="fas fa-download" style="color: #267E15;" ></i></a> / ';
                $button = $button. '<button class="btn btn-sm" title="Editar archivo" data-toggle="modal" onclick="getManual('.strval($d->id).')" data-target="#modalNuevoM"><i class="fas fa-pencil" style="color: #267E15;"></i></button>';
                if($d->estatus == 1){
                    $button = $button.'/<button data-toggle="modal" onclick="deleteManual('.strval($d->id).')" data-placement="top" title="Eliminar archivo" class="btn btn-sm" data-toggle="modal" data-target="#modal_delete"><i class="fas fa-trash" style="color: #ad0b00;" ></i></button>';
                }
    ...
    public function getManual(Request $request){
        $roles = DB::connection('mysql')->table('roles')->select('id','name')->get();
