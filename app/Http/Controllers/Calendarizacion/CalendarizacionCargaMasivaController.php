                    ...
                    if (count($filearray) <= 0) {
                        return redirect()->back()->withErrors(['error' => 'El excel esta vacio']);
                            ...
                            foreach ($arraypresupuesto as $key => $value) {
                                ...
                                $valuepresupuesto = TechosFinancieros::select()->where('clv_upp', $arraysplit[0])->where('ejercicio', $ejercicio[$helperejercicio])->where('tipo', $tipoFondo)->where('clv_fondo', $arraysplit[2])->value('presupuesto')->where('deleted_at',NULL);
            ...
            //Validaciones para usuarios upps 
                ...
                if ($xlsx = SimpleXLSX::parse($request->file)) {
                    ...
                    foreach ($arraypresupuesto as $key => $value) {
                        ...
                        $valuepresupuesto = TechosFinancieros::select()->where('clv_upp', $arraysplit[0])->where('tipo', $tipoFondo)->where('ejercicio', $ejercicio[$helperejercicio])->where('clv_fondo', $arraysplit[2])->value('presupuesto')->where('deleted_at',NULL);
                        if ($valuepresupuesto != $value) {
